...
function inject (bot) {
  ...
  function parseEntityVelocity (velocity) {
    const parsedVelocity = new Vec3(velocity.x, velocity.y, velocity.z)
    return bot.supportFeature('entityVelocityIsLpVec3')
      ? parsedVelocity
      : conv.fromNotchVelocity(parsedVelocity)
  }
  ...
  bot._client.on('spawn_entity_living', (packet) => {
    ...
    entity.velocity.update(parseEntityVelocity(packet.velocity))
  ...
  bot._client.on('entity_velocity', (packet) => {
    // entity velocity
    const entity = fetchEntity(packet.entityId)
    updateEntityVelocity(entity, parseEntityVelocity(packet.velocity))
  })
